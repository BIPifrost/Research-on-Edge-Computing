...
f_max = 1.5e9;            % CPU 1.5 GHz1.5e9the maximum CPU-cycle frequency of mobile device (in Hz)
...
W = L * X;                % CPU the number of CPU cycles needed on processing one task
...
M = 8;                    % MEC the number of MEC servers
...
min_distance = 10;        % MEC the minimum distance from mobile device to MEC server
max_distance = 50;        % MEC the maximum distance from mobile device to MEC server
...
B_hat = zeros(T, N);               % B- the virtual battery energy level ($B_hat = B - theta$)
...
f = zeros(T, N);                   % CPU the CPU-cycle frequency of local execution (in Hz)
...
while t <= T
    ...
    remained_connects = max_connects * ones(M, 1);    % MEC the available connections of MEC servers
...
% generate the virtual battery energy level
    ...
    gamma = exprnd(1, N, M);
    h_mat = g0 * gamma .* power(d0 ./ distances, 4); % h
                ...
                if remained_connects(j) == 0
                    ...
                    % for those unlucky mobile devices i', set J_s(i', j) as inf (i' are in currently device_server_pairs(: , j) now)
                        ...
                        for idx = 1: numel(residual_is)
                            ...
                            % Remove the stale queue entry on server j before re-queuing residual_i.
                            device_server_pairs(device_server_pairs(:, 1) == residual_i & device_server_pairs(:, 2) == j, :) = [];
