...
% ---------- defaults ----------
def = struct();
def.seed_env = 20260307;
def.seed_policy = 20260308;
def.k = 1e-28;
def.tau = 0.002;
def.phi = 0.002;
def.omega = 1e6;
...
def.make_plot = false;
def.plot_title = 'eps-greedy mode selection';
def.save_plot = false;
def.output_prefix = 'eps_basic';
...
B_hat = zeros(T, N);
e = zeros(T, N);
chosen_mode = zeros(T, N); % 1 local, 2 remote, 3 drop, 4 no task
...
% Use independent streams: env randomness is decoupled from policy randomness.
env_stream = RandStream('mt19937ar', 'Seed', p.seed_env);
policy_stream = RandStream('mt19937ar', 'Seed', p.seed_policy);
...
for t = 1:T
    ...
    distances = p.min_distance + (p.max_distance - p.min_distance) .* rand(env_stream, N, M);
    gamma = -log(max(rand(env_stream, N, M), realmin));
    ...
    for i = 1:N
        E_H_t = p.E_H_max * rand(env_stream);
        if B_hat(t, i) <= 0
            e(t, i) = E_H_t;
        end

        zeta = rand(env_stream) <= p.rho;
        if ~zeta
            chosen_mode(t, i) = 4;
            continue;
        end

        % step 1.2.1: local execution
        ...
        % step 1.2.2: MEC execution candidate for each server
        for j = 1:M
            h = h_mat(i, j);
            ...
            if p_L <= p_U
                vb = B_hat(t, i);
                y = @(x) vb * log2(1 + h * x / p.sigma) + h * (p.V - vb * x) / log(2) / (p.sigma + h * x);
        ...
        if rand(policy_stream) <= p.eps
            ...
            if remained_connects(j) >= 1
                ...
                if min(J_s(i, :)) ~= inf
                    ...
                    device_server_pairs = [device_server_pairs; [i, second_min_j, second_min_Js]]; %#ok<AGROW>
                else
                    J_d = p.V * p.phi;
                    [~, mode2] = min([J_m(i), inf, J_d]);
                    if mode2 == 1
                        chosen_mode(t, i) = 1;
                        final_chosen_cost(t, i) = mobile_exe_cost(t, i);
                        final_chosen_E(t, i) = mobile_exe_E(t, i);
                ...
                elseif remained_connects(j) == 0
                    ...
                    for idx = 1:numel(residual_is)
                        ...
                        elseif mode4 == (M + 2)
                            ...
                            chosen_mode(t, ii) = 2;
                            device_server_pairs = [device_server_pairs; [ii, mode4 - 1, J_s(ii, mode4 - 1)]]; %#ok<AGROW>
                        end
                    end
                end
            end
...
xlabel('Time Slot'); ylabel('Ratio'); grid on;

subplot(1, 2, 2);
bar(result.mode_ratio_overall, 'FaceColor', [0.2 0.5 0.85]);
xticks([1, 2, 3]); xticklabels({'Local', 'Remote', 'Drop'});
ylim([0, 1]);
title('Overall Mode Ratio');
ylabel('Ratio'); grid on;
sgtitle(fig_title);
end
