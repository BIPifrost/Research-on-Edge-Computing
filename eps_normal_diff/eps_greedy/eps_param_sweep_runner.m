...
if ~isfield(opts, 'T'), opts.T = 800; end
if ~isfield(opts, 'seed_env'), opts.seed_env = 20260307; end
if ~isfield(opts, 'seed_policy'), opts.seed_policy = 20260308; end
...
for k = 1:numel(values)
    cfg = struct();
    cfg.T = opts.T;
    cfg.seed_env = opts.seed_env;
    cfg.seed_policy = opts.seed_policy;
    cfg.make_plot = false;
    cfg = apply_overrides(cfg, opts.extra_cfg);

    cfg.(param_name) = values(k);

    % Keep tau_d physically plausible when sweeping L.
    if strcmp(param_name, 'L')
        cfg.tau_d = min(max(0.002, 0.002 * (values(k) / 1000)), 0.010);
    end

    results{k} = eps_greedy_logco_basic(cfg);
    overall(k, :) = results{k}.mode_ratio_overall;
end

figure('Name', ['eps_', param_name], 'Position', [120 120 1380 520]);
...
xlabel(param_label); ylabel('Ratio'); ylim([0, 1]); grid on;
...
xlabel('Time Slot'); ylabel('Ratio'); ylim([0, 1]); grid on;
sgtitle(compose_sgtitle(param_label, opts.title_suffix));

prefix = fullfile(opts.output_dir, ['eps_', param_name]);
if opts.save_plot
    saveas(gcf, [prefix, '.png']);
    savefig(gcf, [prefix, '.fig']);
end
