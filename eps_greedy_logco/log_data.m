...
%% =========================================================================
function run_simulation_and_log(use_bisection, seed, filename, T)
    rng(seed);
    opt = optimset('Display', 'none');
    
    fid = fopen(filename, 'w');
    fprintf(fid, 'Time,Device,B_hat,J_m,Best_Js,Target_MEC,J_d,p_L,p_U,p_0,Init_Mode,Final_Mode,Cost,Energy\n');
    ...
    for t = 1: T
        ...
        log_best_Js = zeros(N, 1); log_best_MEC = zeros(N, 1);
        log_p_L = zeros(N, 1); log_p_U = zeros(N, 1); log_p_0 = zeros(N, 1);
        log_init_mode = zeros(N, 1);
        ...
        for i = 1: N
            ...
            best_j_tmp = 1; min_Js_tmp = inf;
            p_L_tmp = 0; p_U_tmp = 0; p_0_tmp = 0;
            ...
            for j = 1: M
                ...
                if E_tmp >= E_min
                    ...
                    if use_bisection
                        a_min = (omega * E_min) / (L * log(2)) - sigma / h;
                ...
                if E_tmp >= E_max
                    ...
                    if use_bisection
                        a_max = (omega * E_max) / (L * log(2)) - sigma / h;
                ...
                if p_L <= p_U
                    ...
                    if use_bisection
                        p_0 = my_bisection(y_p0, 0, p_tx_max, 1e-7);
                ...
                if J_s(i, j) < min_Js_tmp
                    min_Js_tmp = J_s(i, j);
                    best_j_tmp = j;
                    p_L_tmp = p_L; p_U_tmp = p_U; p_0_tmp = p_0;
                end
            end
            
            log_best_Js(i) = min_Js_tmp;
            log_best_MEC(i) = best_j_tmp;
            log_p_L(i) = p_L_tmp; log_p_U(i) = p_U_tmp; log_p_0(i) = p_0_tmp;
            
            J_d = V * phi;
            [~, mode] = min([J_m(i), min_Js_tmp, J_d]);
            log_init_mode(i) = mode;
            ...
            elseif mode == 3
                ...
                chosen_mode(t, i) = 2;
                device_server_pairs = [device_server_pairs; [i, best_j_tmp, min_Js_tmp]];
            end
        end
        ...
        for i = 1: N
            if log_init_mode(i) ~= 4
                fprintf(fid, '%d,%d,%.6f,%.6e,%.6e,%d,%.6e,%.6e,%.6e,%.6e,%d,%d,%.6e,%.6e\n', ...
                    t, i, B_hat(t,i), J_m(i), log_best_Js(i), log_best_MEC(i), V*phi, ...
                    log_p_L(i), log_p_U(i), log_p_0(i), log_init_mode(i), chosen_mode(t, i), final_chosen_cost(t, i), final_chosen_E(t, i));
            end
        end
