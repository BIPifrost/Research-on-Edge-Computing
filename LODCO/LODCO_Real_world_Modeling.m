function [avg_cost, avg_battery] = LODCO_Real_world_Modeling(V_input)
...
V = V_input;                 % the weight of penalty (the control parameter introduced by Lyapunov Optimization)
...
while t <= T
    ...
    if zeta == 0
        ...
        h_random = exprnd(g0 / power(d, 4));
        h_current = corr_coeff * h_current + (1 - corr_coeff) * h_random;
        h = h_current;
        ...
        if p_L <= p_U
            ...
            Header_Size = 400; 
            L_real = L + Header_Size;
            Goodput_Ratio = 0.7;
            RTT = 0.005;
            R_phy = omega * log2(1 + h*p(t)/sigma); 
            R_app = R_phy * Goodput_Ratio;          
            cost(t, 2) = (L_real / R_app) + RTT;
            % -------------------------------------------
            
            % calculate the energy consumption of MEC server execution
            E(t, 2) = p(t) * (L_real / R_app); 
...
save('data_original.mat', 'time_history', 'B', 'average_cost', 'average_ratio');
disp('Original data saved to data_original.mat');
avg_cost = average_cost(end);
avg_battery = mean(B);
end
