    ...
    if zeta == 0
       ...
       %% step 2.2: solve the optimization problem $\mathcal{P}_{SE}$ (p(t) > 0)
        ...
        if E_tmp >= E_min
            ...
            y_min_func = @(x) x * L - omega * log2(1 + h*x/sigma) * E_min;
            lb = bisect_lb; ub = bisect_ub;
            while (ub - lb) > bisect_tol
                mid = (lb + ub) / 2;
                if y_min_func(mid) > 0 
                    ub = mid;
                else
        ...
        if E_tmp >= E_max
            ...
            y_max_func = @(x) x * L - omega * log2(1 + h*x/sigma) * E_max;
            lb = bisect_lb; ub = bisect_ub;
            while (ub - lb) > bisect_tol
                mid = (lb + ub) / 2;
                if y_max_func(mid) > 0
        ...
        if p_L <= p_U
            ...
            if B_hat(t) >= 0
                ...
                y_main = @(x) virtual_battery * log2(1 + h*x/sigma) + ...
                    h * (V - virtual_battery*x) / log(2) / (sigma + h*x);
                
                lb = 0; ub = 100;
                y_lb = y_main(lb); y_ub = y_main(ub);
                
                if y_lb * y_ub > 0
                    if abs(y_lb) < abs(y_ub), p_0 = lb; else, p_0 = ub; end
                else
                    while (ub - lb) > bisect_tol
                        mid = (lb + ub) / 2;
                        if y_main(mid) == 0, break; end
                        if y_lb * y_main(mid) < 0
                            ub = mid;
                        else
                            lb = mid;
                            y_lb = y_main(mid);
...
figure
plot(1:T, average_ratio(:, 2), 'Color', [0.8500 0.3250 0.0980]);
hold on

plot(1:T, average_ratio(:, 1), 'Color', [0 0.4470 0.7410], 'LineWidth', 1.5);
hold on

plot(1:T, average_ratio(:, 3), 'Color', [0.9290 0.6940 0.1250]);

legend('MEC server execution', 'mobile execution', 'drop')
title('Evolution of average ratio of chosen modes')
xlabel('time slot')
ylabel('average ratio $\frac{1}{T} \sum_{t=0}^{T-1} \{I_m^t, I_s^t, I_d^t\}$', 'Interpreter','latex')
ylim([0, 1]);
%% 
num = T - sum(chosen_mode == 4);
disp(['Task arrival frequency: ', num2str(num/T)]);
mode1 = sum(chosen_mode==1) / (num);
disp(['Local execution ratio: ', num2str(mode1)]);
mode2 = sum(chosen_mode==2) / (num);
disp(['Offloading execution ratio: ', num2str(mode2)]);
mode3 = sum(chosen_mode==3) / (num);
disp(['Dropped task ratio: ', num2str(mode3)]);

save('data_optimized.mat', 'time_history', 'B', 'average_cost', 'average_ratio');
disp('Optimized data saved to data_optimized.mat');
