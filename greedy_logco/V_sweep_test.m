...
p_tx_max = 1; f_max = 1.5e9; E_max = 0.002; L = 1000; X = 737.5; W = L * X;
E_H_max = 48e-6; p_H = E_H_max / (2*tau); g0 = power(10, -4); d0 = 1;
N = 10; M = 8; T = 200; tau_d = 0.002; d = 50; E_min = 0.02e-3; rho = 0.6;
max_connects = 4; min_distance = 10; max_distance = 50;
E_max_hat = min(max(k * W * (f_max)^2, p_tx_max * tau), E_max);



V_array = [1e-6, 5e-6, 1e-5, 5e-5, 1e-4, 5e-4, 1e-3];
num_V = length(V_array);


avg_delay_results = zeros(num_V, 1);
avg_energy_results = zeros(num_V, 1);
avg_battery_results = zeros(num_V, 1);

disp('>>> Starting V-parameter sweep with bisection acceleration...');


for v_idx = 1:num_V
    V = V_array(v_idx);
    disp(['===========================================']);
    disp(['>>> Running case ', num2str(v_idx), '/', num2str(num_V), ': V = ', num2str(V)]);
    ...
    t = 1;
    while t <= T
        device_server_pairs = []; remained_connects = max_connects * ones(M, 1);
        J_m = zeros(N, 1); J_s = zeros(N, M); p_mat = zeros(N, M);
        server_cost_mat = zeros(N, M); server_E_mat = zeros(N, M);
        ...
        for i = 1: N
            ...
            if zeta == 0
                ...
                if f_L <= f_U
                    ...
                    f(t, i) = 0; mobile_exe_cost(t, i) = 0; mobile_exe_E(t, i) = 0; J_m(i) = inf;
                end
                ...
                for j = 1: M
                    ...
                    if E_tmp >= E_min
                        ...
                        p_E_min = bisection_solve(y1, 1e-9, 1, 1e-6);
                    ...
                    if E_tmp >= E_max
                        ...
                        p_E_max = bisection_solve(y2, 1e-9, 1, 1e-6);
                    ...
                    if p_L <= p_U
                        ...
                        p_0 = bisection_solve(y3, 0, 1, 1e-6);
                        ...
                        p_mat(i, j) = 0; server_cost_mat(i, j) = 0; server_E_mat(i, j) = 0; J_s(i, j) = inf;
                    end
                end
            ...
            for j = 1: M
                ...
                if remained_connects(j) >= length(is)
                    ...
                    if remained_connects(j) == 0
                        device_server_pairs(device_server_pairs(:, 2) == j, :) = []; J_s(is, j) = inf;
                        for idx = 1: numel(is)
                            i = is(idx);
                            [~, mode] = min([J_m(i), J_s(i, :), J_d]);
                            if mode == 1
                        ...
                        residual_is = device_server_pairs(device_server_pairs(:, 2) == j, 1); J_s(residual_is, j) = inf;
    ...
    valid_tasks_mask = (chosen_mode ~= 4);
    
    avg_delay_results(v_idx) = sum(final_chosen_cost(valid_tasks_mask)) / sum(valid_tasks_mask(:));
    avg_energy_results(v_idx) = sum(final_chosen_E(valid_tasks_mask)) / sum(valid_tasks_mask(:));
    avg_battery_results(v_idx) = mean(B(:));
...
figure('Name', 'Delay vs V', 'Position', [100, 200, 500, 400]);
semilogx(V_array, avg_delay_results, '-o', 'LineWidth', 2, 'MarkerSize', 6, 'MarkerFaceColor', 'b');
grid on;
title('Average Delay vs Control Parameter V');
xlabel('Control Parameter V');
ylabel('System Average Delay (s)');


figure('Name', 'Energy vs V', 'Position', [650, 200, 500, 400]);
semilogx(V_array, avg_energy_results, '-s', 'LineWidth', 2, 'MarkerSize', 6, 'MarkerFaceColor', 'r', 'Color', 'r');
grid on;
title('Average Energy vs Control Parameter V');
xlabel('Control Parameter V');
ylabel('System Average Energy (J)');


figure('Name', 'Battery Level vs V', 'Position', [1200, 200, 500, 400]);
semilogx(V_array, avg_battery_results, '-^', 'LineWidth', 2, 'MarkerSize', 6, 'MarkerFaceColor', 'g', 'Color', 'g');
grid on;
title('Average Battery Level vs Control Parameter V');
xlabel('Control Parameter V');
ylabel('Average Battery Level (J)');


function root = bisection_solve(func, left, right, tol)
    max_iter = 100; iter = 0;
    while sign(func(left)) == sign(func(right)) && iter < max_iter
        right = right * 2;
        iter = iter + 1;
    end
    while (right - left) > tol
        ...
        if f_mid == 0
            break;
        elseif sign(f_mid) == sign(func(left))
            left = mid;
        else
            right = mid;
        end
